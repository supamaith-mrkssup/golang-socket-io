...
import (
	...
	internalRedis "go-chat-api/internal/db/redis"
	"strings"
	"sync"
)
...
func newRedisBroadcast(nsp string, opts *RedisAdapterOptions) (*redisBroadcast, error) {
	...
	subConn := &redis.PubSubConn{Conn: internalRedis.RedisPool.Get()}
	pubConn := &redis.PubSubConn{Conn: internalRedis.RedisPool.Get()}
...
func (bc *redisBroadcast) AllRooms() []string {
	...
	_, err := internalRedis.RedisPool.Get().Do("PUBLISH", bc.reqChannel, reqJSON)
	if err != nil {
...
func (bc *redisBroadcast) SendAll(event string, args ...interface{}) {
	logrus.Infoln("emit connection send all start")
	bc.lock.RLock()
	defer bc.lock.RUnlock()
	...
	logrus.Infoln("emit connection send all end")
	logrus.Infoln("public redis broadcast  send all start")
	bc.publishMessage("", event, args...)
	logrus.Infoln("public redis broadcast  send all end")
}
...
func (bc *redisBroadcast) getNumSub(channel string) (int, error) {
	rs, err := internalRedis.RedisPool.Get().Do("PUBSUB", "NUMSUB", channel)
	if err != nil {
		return 0, err
	}
...
func (bc *redisBroadcast) publish(channel string, msg interface{}) {
	...
	_, err = internalRedis.RedisPool.Get().Do("PUBLISH", channel, resJSON)
	if err != nil {
		return
	}
}
...
func (bc *redisBroadcast) publishMessage(room string, event string, args ...interface{}) {
	...
	_, err = internalRedis.RedisPool.Get().Do("PUBLISH", bc.key, bcMessageJSON)
	if err != nil {
		logrus.Errorln("Publish error.", err)
		return
	}
}
