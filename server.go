...
import (
	...
	"go-chat-api/internal/pkg/gochat-socket/engineio"
	"net/http"
)
...
func (s *Server) Adapter(opts *RedisAdapterOptions) (bool, error) {
	...
	conn, err := redis.Dial(opts.Network, opts.getAddr(), redis.DialPassword(opts.Password))
	if err != nil {
		return false, err
	}
